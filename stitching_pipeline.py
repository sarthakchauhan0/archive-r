...
def main():
    ...
    metadata['iso_alpha'] = metadata['ccode'].map(lambda x: cow_crosswalk.get(int(x), ('UNK', 'Others'))[0])
    metadata['region'] = metadata['ccode'].map(lambda x: cow_crosswalk.get(int(x), ('UNK', 'Others'))[1])
